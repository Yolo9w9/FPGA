...
//由编码规则可知，当信码序列出现“101”时，Miller码出现最大脉冲宽度为两个码元周期，而信码出现连“0时，它的最小脉冲宽度为一个码元周期，这一性质可用于进行误码检测。
always @(posedge clk)
begin
	...
	begin
		...
		else if(dcode11[1:0] == 2'b01 || dcode11[1:0] == 2'b10 )
		begin
			decode <= 1;
			fail <= 0;
		end
		else if(dcode11[1:0] == 2'b11  || dcode11[1:0] == 2'b00  )
		begin
			decode <= 0;
			fail <= 0;
		end
